NAME_OF_FILE_TO_GENERATE = "car_ped_parallelcomposition.prism"  #Add prism to the end of this file
...
with open(NAME_OF_FILE_TO_GENERATE, "w") as filename:
    filename.write("dtmc \n\n")
    filename.write(f"module car{NUM_CAR_STATES}_ped{NUM_PED_STATES}_parallelcomposition \n\n")
    
    filename.write(f"\t c : [1..{NUM_CAR_STATES}] init 1; \n")
    filename.write(f"\t p : [1..{NUM_PED_STATES}] init 1; \n")
    ...
    for car in range(1, NUM_CAR_STATES + 1):
        for ped in range(1, NUM_PED_STATES + 1):
            ...
            if car == 5 and ped == 5:
                ...
                filename.write(f"\t \t []           c={car} & p={ped} -> {round(P_PED_STAY, 2)}:(c'={car}) & (p'={ped}) + {round(P_PED_FORW, 2)}: (c'={car}) & (p'={ped+1}); \n")
                # filename.write(f"\t \t [accelerate] c={car} & p={ped} -> {round(P_PED_STAY, 2)}:(c'={car}) & (p'={ped}) + {round(P_PED_FORW, 2)}: (c'={car}) & (p'={ped+1}); \n")
                # filename.write(f"\t \t      [brake] c={car} & p={ped} -> {round(P_PED_STAY, 2)}:(c'={car}) & (p'={ped}) + {round(P_PED_FORW, 2)}: (c'={car}) & (p'={ped+1}); \n")

            elif ped == 5:
                filename.write(f"\t \t [accelerate] c={car} & p={ped} -> {round(P_ACC_STAY, 2)}:(c'={car}) & (p'={ped}) + {round(P_ACC_FORW, 2)}: (c'={car+1}) & (p'={ped}); \n")
                filename.write(f"\t \t [brake]      c={car} & p={ped} -> {round(P_BRK_STAY, 2)}:(c'={car}) & (p'={ped}) + {round(P_BRK_FORW, 2)}: (c'={car+1}) & (p'={ped}); \n")
...
print(f" = = = = = Done generating {NAME_OF_FILE_TO_GENERATE} = = = = =")
